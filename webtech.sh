...
then
	...
	echo "=================="
	echo "[00m"
	nmap -A $1 -p 80,81,82,8008,8080,443,1443,2443,3443,4443,5443,6443,7443,8443,9443 -oN nmap-$1.log|tee webtech-$1.log
	...
	echo "[00m"
	while IFS=, read -r webport; do
		wappalyzer http://$1:$webport|wappaligner|tee -a webtech-$1.log
	done < $1-http-tmp.log
	echo
	while IFS=, read -r webport; do
		wappalyzer https://$1:$webport|wappaligner|tee -a webtech-$1.log
	done < $1-https-tmp.log
	...
	echo "=====================[92m"
	while IFS=, read -r webport; do
		wafw00f http://$1:$webport|tee -a webtech-$1.log
	done < $1-http-tmp.log
	while IFS=, read -r webport; do
		wafw00f https://$1:$webport|tee -a webtech-$1.log
	done < $1-https-tmp.log
